...
export class RdwService {
  ...
  getCarByLicensePlateNumber(
    licensePlateNumber: string
  ): Observable<RdwCarResponse> {
    return this.http
      .get<RdwCarResponse[]>(
        `${this.generalUrl}?kenteken=${licensePlateNumber}`,
        httpOptions
      )
      .pipe(
        map((value: RdwCarResponse[]) => {
          return {
            ...
            merk: value[0].merk,
            handelsbenaming: value[0].handelsbenaming
          };
        })
      );
  }

  getFuelByLicensePlateNumber(
    licensePlateNumber: string
  ): Observable<RdwFuelResponse> {
    return this.http
      .get<RdwFuelResponse[]>(
        `${this.fuelUrl}?kenteken=${licensePlateNumber}`,
        httpOptions
      )
      .pipe(
